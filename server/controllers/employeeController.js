...
const register = async (req, res) => {
  try {
    ...
    res.status(201).json({ employee: { id: employee._id, firstName, lastName, email, role, department }, token });
  } catch (error) {
...
const login = async (req, res) => {
  try {
    ...
    const token = jwt.sign({ id: employee._id }, process.env.JWT_SECRET, { expiresIn: process.env.JWT_EXPIRE });
    res.json({ employee: { id: employee._id, firstName: employee.firstName, lastName: employee.lastName, email, role: employee.role }, token });
  } catch (error) {
    res.status(400).json({ error: error.message });
  }
};
